...
function gridReload(page) {
    ...
    grid.jqGrid('setGridParam', {
        url: "/licencias/medicas/revision/lst?rand=" + Math.random(),
        postData: datos,
        page: page
    }).trigger("reloadGrid");
}
...
function listar() {
    ...
    datos['claseEstados'] = 'danger, warning';
    const grilla = jQuery("#listarDatos");
    armarGrilla(datos, grilla, '#pager2', '#LstDatos');
...
function listar2() {
    ...
    let datos = $("#formbusqueda").serializeObject();
    datos['claseEstados'] = 'default,success';
    const grilla = jQuery("#listarDatos2");
    armarGrilla(datos, grilla, '#pager22', '#LstDatos2');
    ...
    grilla.jqGrid(
        {
            ...
            postData: datos,
            datatype: "json",
            colNames: ['Id', 'Agente','Escuela', 'Inicio', 'Fin', 'Duraci\u00f3n', 'Art\u00edculos', 'Diagn\u00f3stico',
                'Estado p\u00FAblico', 'Fecha de env\u00edo', 'Autorizante', 'Junta', 'Editar','Auditoria'],
            colModel: [
                {name: 'Id', index: 'Id', width: 10, align: 'center'},
                {name: 'Persona.NombreCompleto.raw', index: 'Persona.NombreCompleto.raw', width: 30, align: 'left'},
                {name: 'Cargos.Escuela.Id', index: 'Cargos.Escuela.Id', width: 20, align: 'center',sortable: false},
                {name: 'Inicio', index: 'Inicio', width: 15, align: 'left'},
                {name: 'Fin', index: 'Fin', width: 15, align: 'left'},
                {name: 'Duracion', index: 'Duracion', width: 15, align: 'left'},
                {name: 'Articulo.Id', index: 'Articulo.Id', width: 20, align: 'left', sortable: false},
                {name: 'Motivo.Id', index: 'Motivo.Id', width: 20, align: 'left'},
                {name: 'Estado.NombrePublico', index: 'Estado.NombrePublico', width: 15, align: 'left', sortable: false},
                {name: 'FechaEnvio', index: 'FechaEnvio', width: 17, align: 'left', sortable: false},
                {name: 'Aprobante.Id', index: 'Aprobante.Id', width: 15, align: 'left', sortable: false},
                {name: 'Junta', index: 'Junta', width: 10, align: 'left', sortable: false},
                {name: 'Editar', index: 'Editar', width: 10, align: 'left', sortable: false},
                {name: 'Auditoria', index: 'Auditoria', width: 10, align: 'center', sortable: false}
            ],
            rowNum: 20,
