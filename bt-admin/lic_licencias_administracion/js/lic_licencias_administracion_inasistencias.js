		...
		if (!listados[id]) {
			switch (id) {
				...
				case 'tabApCondicional':
					...
					break;
				case 'tabInjustificada':
					setTimeout(()=>listar6(), 500);
					break;
			}
		} else {
			switch (id) {
				...
				case 'tabApCondicional':
					...
					break;
				case 'tabInjustificada':
					setTimeout(()=>jQuery("#listarDatos6").setGridWidth($('#LstDatos6').width()), 500);
					break;
			}
		}
...
function gridReload(page) {
	...
	grid.jqGrid('setGridParam', {
		url: "/licencias/inasistencias/revision/lst?rand=" + Math.random(),
		postData: datos,
		page: page
	}).trigger("reloadGrid");
}
...
function gridReload6(page) {
	const grid = jQuery("#listarDatos6");
	grid.setGridParam({postData: null});
	const form = $("#formbusqueda");
	let datos = form.serializeObject();
	/*datos['claseEstados'] = 'danger';*/
	 datos['IdEstado'] = 24;
...
function listar6() {
	listados['tabInasistencia'] = true;
	let datos = $("#formbusqueda").serializeObject();
	/*datos['claseEstados'] = 'danger';*/
	datos['IdEstado'] = 24;
	const grilla = jQuery("#listarDatos6");
	armarGrilla(datos, grilla, '#pager26', '#LstDatos6');
	...
	grilla.jqGrid(
		{
			...
			postData: datos,
			datatype: "json",
			colNames: ['Id', 'Agente','Escuela', 'Inicio', 'Fin', 'Duraci\u00f3n', 'Motivo', 'Estado p\u00FAblico', 'Fecha de env\u00edo', 'Editar', 'Auditoria'],
			colModel: [
				...
				{name: 'Duracion', index: 'Duracion', width: 15, align: 'center'},
				{name: 'Motivo.Id', index: 'Motivo.Id', width: 50, align: 'left'},
				{name: 'Estado.NombrePublico', index: 'Estado.NombrePublico', width: 20, align: 'center', sortable: false},
				{name: 'FechaEnvio', index: 'FechaEnvio', width: 20, align: 'center', sortable: false},
