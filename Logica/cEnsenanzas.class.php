<?php 
include(DIR_CLASES_DB."cEnsenanzas.db.php");

class cEnsenanzas extends cEnsenanzasdb
{
	...
	public function BusquedaAvanzada($datos,&$resultado,&$numfilas)
	{
		...
		if(isset($datos['TipoEnsenanza']) && $datos['TipoEnsenanza']!="")
		{
			$sparam['TipoEnsenanza']= $datos['TipoEnsenanza'];
			$sparam['xTipoEnsenanza']= 1;
		}
	...
	public function Insertar($datos,&$codigoinsertado)
	{
		...
		$oAuditoriasEnsenanzas = new cAuditoriasEnsenanzas($this->conexion,$this->formato);
		$datos['IdEnsenanza'] = $codigoinsertado;
		$datos['Accion'] = INSERTAR;
		$datos['AltaUsuario'] = $datos['AltaUsuario'];
		$datos['AltaFecha'] = $datos['AltaFecha'];
		if(!$oAuditoriasEnsenanzas->InsertarLog($datos,$codigoInsertadolog))
			return false;
	...
	public function Modificar($datos)
	{
		...
		if(!$oAuditoriasEnsenanzas->InsertarLog($datosRegistro,$codigoInsertadolog))
			return false;
	...
	public function Eliminar($datos)
	{
		...
		if(!$oAuditoriasEnsenanzas->InsertarLog($datosLog,$codigoInsertadolog))
			return false;

		$datosmodif['IdEnsenanza'] = $datos['IdEnsenanza'];
...
//-----------------------------------------------------------------------------------------
	...
	private function _ValidarInsertar($datos)
	{
		...
		$datosbuscar['IdEnsenanzaExterno'] = $datos['IdEnsenanzaExterno'];	
		if(!$this->BusquedaAvanzada($datosbuscar,$resultado,$numfilas))
			return false;	
			
		if($numfilas>0)
		{
			FuncionesPHPLocal::MostrarMensaje($this->conexion,MSG_ERRGRAVE,"Error, ya existe el id enseñanza.",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__),array("formato"=>$this->formato));
			return false;
		}		
	...
	private function _ValidarModificar($datos,&$datosRegistro)
	{
		...
		if($numfilas>0)
		{
			...
			if($fila['IdEnsenanza']!=$datosRegistro['IdEnsenanza'])
			{
				FuncionesPHPLocal::MostrarMensaje($this->conexion,MSG_ERRGRAVE,"Error, ya existe el id enseñanza.",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__),array("formato"=>$this->formato));
				return false;
			}
	...
	private function _ValidarDatosVacios($datos)
	{
		...
		if (!isset($datos['TipoEnsenanza']) || $datos['TipoEnsenanza']=="")
		{
			FuncionesPHPLocal::MostrarMensaje($this->conexion,MSG_ERRGRAVE,"Debe ingresar un tipo enseñanza",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__),array("formato"=>$this->formato));
			return false;
		}
