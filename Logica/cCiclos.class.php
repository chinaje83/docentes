<?php 
include(DIR_CLASES_DB."cCiclos.db.php");
class cCiclos extends cCiclosdb
{
	/**
	 * Constructor de la clase cCiclos.
	 *
	...
	/**
	 * Destructor de la clase cCiclos.
	 */
	function __destruct(){
		parent::__destruct();
	}
	...
	public function BusquedaAvanzada($datos,&$resultado,&$numfilas): bool
	{
		...
		if(isset($datos['IdCicloExterno']) && $datos['IdCicloExterno']!="")
		{
			$sparam['IdCicloExterno']= $datos['IdCicloExterno'];
			$sparam['xIdCicloExterno']= 1;
		}
	...
	public function Insertar($datos,&$codigoInsertado): bool
	{
		...
		if (!parent::Insertar($datos,$codigoInsertado))
			return false;
		$oAuditoriasCiclos = new cAuditoriasCiclos($this->conexion,$this->formato);
		$datos['IdCiclo'] = $codigoInsertado;
		$datos['Accion'] = INSERTAR;
		$datos['AltaUsuario'] = $datos['AltaUsuario'];
		$datos['AltaFecha'] = $datos['AltaFecha'];
		if(!$oAuditoriasCiclos->InsertarLog($datos,$codigoInsertadolog))
			return false;
		return true;
	}
	...
	public function Modificar($datos): bool
	{
		...
		if(!$oAuditoriasCiclos->InsertarLog($datosRegistro,$codigoInsertadolog))
			return false;
		return true;
	}
	...
	public function Eliminar($datos): bool
	{
		...
		if(!$oAuditoriasCiclos->InsertarLog($datosLog,$codigoInsertadolog))
			return false;
		$datosmodif['IdCiclo'] = $datos['IdCiclo'];
    ...
    public function AsignarOrientacion(array $datos): bool
    {

        $oAuditoriasCiclos = new cAuditoriasCiclos($this->conexion,$this->formato);
        $datosRegistro['Accion'] = MODIFICACION;
        $datosRegistro['IdCiclo'] = $datos['IdCiclo'];
        if(!$oAuditoriasCiclos->InsertarLog($datosRegistro,$codigoInsertadolog))
            return false;

        $datos['AsignaOrientacion'] = 1;
        if (!parent::PermitirOrientacion($datos))
            return false;
    ...
    public function DesasignarOrientacion(array $datos): bool
    {
        $oAuditoriasCiclos = new cAuditoriasCiclos($this->conexion,$this->formato);
        $datosRegistro['Accion'] = MODIFICACION;
        $datosRegistro['IdCiclo'] = $datos['IdCiclo'];
...
//-----------------------------------------------------------------------------------------
	...
	private function _ValidarDatosVacios($datos)
	{
		...
		if (isset($datos['IdCicloExterno']) && $datos['IdCicloExterno']!="")
		{
			if (!FuncionesPHPLocal::ValidarContenido($this->conexion,$datos['IdCicloExterno'],"NumericoEntero"))
			{
