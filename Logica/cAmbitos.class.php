<?php 
include(DIR_CLASES_DB."cAmbitos.db.php");
class cAmbitos extends cAmbitosdb
{
	/**
	...
	public function BusquedaAvanzada($datos,&$resultado,&$numfilas): bool
	{
		...
		if(isset($datos['IdAmbitoExterno']) && $datos['IdAmbitoExterno']!="")
		{
			$sparam['IdAmbitoExterno']= $datos['IdAmbitoExterno'];
			$sparam['xIdAmbitoExterno']= 1;
		}
	...
	public function Insertar($datos,&$codigoInsertado): bool
	{
		...
		if (!parent::Insertar($datos,$codigoInsertado))
			return false;
		$oAuditoriasAmbitos = new cAuditoriasAmbitos($this->conexion,$this->formato);
		$datos['IdAmbito'] = $codigoInsertado;
		$datos['Accion'] = INSERTAR;
		$datos['AltaUsuario'] = $datos['AltaUsuario'];
		$datos['AltaFecha'] = $datos['AltaFecha'];
		if(!$oAuditoriasAmbitos->InsertarLog($datos,$codigoInsertadolog))
			return false;
		return true;
	}
	...
	public function Modificar($datos): bool
	{
		...
		if(!$oAuditoriasAmbitos->InsertarLog($datosRegistro,$codigoInsertadolog))
			return false;
		return true;
	}
	...
	public function Eliminar($datos): bool
	{
		...
		if(!$oAuditoriasAmbitos->InsertarLog($datosLog,$codigoInsertadolog))
			return false;
		$datosmodif['IdAmbito'] = $datos['IdAmbito'];
...
//-----------------------------------------------------------------------------------------
	...
	private function _ValidarDatosVacios($datos)
	{
		...
		if (isset($datos['IdAmbitoExterno']) && $datos['IdAmbitoExterno']!="")
		{
			if (!FuncionesPHPLocal::ValidarContenido($this->conexion,$datos['IdAmbitoExterno'],"NumericoEntero"))
			{
