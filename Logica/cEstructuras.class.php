<?php 
include(DIR_CLASES_DB."cEstructuras.db.php");
class cEstructuras extends cEstructurasdb
{
	/**
	 * Constructor de la clase cEstructuras.
	 *
	...
	/**
	 * Destructor de la clase cEstructuras.
	 */
	function __destruct(){
		parent::__destruct();
	}
	...
	public function BusquedaAvanzada($datos,&$resultado,&$numfilas): bool
	{
		...
		if(isset($datos['IdEstructuraExterno']) && $datos['IdEstructuraExterno']!="")
		{
			$sparam['IdEstructuraExterno']= $datos['IdEstructuraExterno'];
			$sparam['xIdEstructuraExterno']= 1;
		}
	...
	public function Insertar($datos,&$codigoInsertado): bool
	{
		...
		if (!parent::Insertar($datos,$codigoInsertado))
			return false;
		$oAuditoriasEstructuras = new cAuditoriasEstructuras($this->conexion,$this->formato);
		$datos['IdEstructura'] = $codigoInsertado;
		$datos['Accion'] = INSERTAR;
		$datos['AltaUsuario'] = $datos['AltaUsuario'];
		$datos['AltaFecha'] = $datos['AltaFecha'];
		if(!$oAuditoriasEstructuras->InsertarLog($datos,$codigoInsertadolog))
			return false;
		return true;
	}
	...
	public function Modificar($datos): bool
	{
		...
		if(!$oAuditoriasEstructuras->InsertarLog($datosRegistro,$codigoInsertadolog))
			return false;
		return true;
	}
	...
	public function Eliminar($datos): bool
	{
		...
		if(!$oAuditoriasEstructuras->InsertarLog($datosLog,$codigoInsertadolog))
			return false;
		$datosmodif['IdEstructura'] = $datos['IdEstructura'];
...
//-----------------------------------------------------------------------------------------
	...
	private function _SetearNull(&$datos): void
	{
		...
		if (!isset($datos['IdEstructuraExterno']) || $datos['IdEstructuraExterno']=="")
			$datos['IdEstructuraExterno']="NULL";
	...
	private function _ValidarDatosVacios($datos)
	{
		...
		if (isset($datos['IdEstructuraExterno']) && $datos['IdEstructuraExterno']!="")
		{
			if (!FuncionesPHPLocal::ValidarContenido($this->conexion,$datos['IdEstructuraExterno'],"NumericoEntero"))
			{
