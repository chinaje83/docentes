<?php 
include(DIR_CLASES_DB."cLocalidades.db.php");
class cLocalidades extends cLocalidadesdb
{
	/**
	 * Constructor de la clase cLocalidades.
	 *
	...
	/**
	 * Destructor de la clase cLocalidades.
	 */
	function __destruct(){
		parent::__destruct();
	}
    ...
    public function BuscarxIdDepartamento($datos,&$resultado,&$numfilas): bool
    {
        $sparam=array(
        ...
        if (!parent::BuscarxIdDepartamento($sparam,$resultado,$numfilas))
            return false;
        return true;
    }
	...
	public function BusquedaAvanzada($datos,&$resultado,&$numfilas): bool
	{
        ...
        {
            $sparam['IdMunicipio']= $datos['IdMunicipio'];
            $sparam['xIdMunicipio']= 1;
        }
    ...
    public function BuscarxMunicipio($datos,&$resultado,&$numfilas): bool
    {
        if (!parent::BuscarxMunicipio($datos,$resultado,$numfilas))
            return false;
        return true;
    }
	...
	public function Insertar($datos,&$codigoInsertado): bool
	{
		...
		if (!parent::Insertar($datos,$codigoInsertado))
			return false;
		$oAuditoriasLocalidades = new cAuditoriasLocalidades($this->conexion,$this->formato);
		$datos['IdLocalidad'] = $codigoInsertado;
		$datos['Accion'] = INSERTAR;
		$datos['AltaUsuario'] = $datos['AltaUsuario'];
		$datos['AltaFecha'] = $datos['AltaFecha'];
		if(!$oAuditoriasLocalidades->InsertarLog($datos,$codigoInsertadolog))
			return false;
		return true;
	}
	...
	public function Modificar($datos): bool
	{
		...
		if(!$oAuditoriasLocalidades->InsertarLog($datosRegistro,$codigoInsertadolog))
			return false;
		return true;
	}
	...
	public function Eliminar($datos): bool
	{
		...
		if(!$oAuditoriasLocalidades->InsertarLog($datosLog,$codigoInsertadolog))
			return false;
		$datosmodif['IdLocalidad'] = $datos['IdLocalidad'];
...
//-----------------------------------------------------------------------------------------
	...
	private function _ValidarDatosVacios($datos)
	{
		...
		if (!isset($datos['IdProvincia']) || $datos['IdProvincia']=="")
		{
			FuncionesPHPLocal::MostrarMensaje($this->conexion,MSG_ERRGRAVE,"Debe ingresar un provincia",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__),array("formato"=>$this->formato));
			return false;
		}
	...
	/*	if (!isset($datos['IdDepartamento']) || $datos['IdDepartamento']=="")
		{
			FuncionesPHPLocal::MostrarMensaje($this->conexion,MSG_ERRGRAVE,"Debe ingresar un departamento",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__),array("formato"=>$this->formato));
			return false;
		}
...
//		if (!$this->conexion->TraerCampo('Departamentos','IdDepartamento',array('IdDepartamento='.$datos['IdDepartamento']),$dato,$numfilas,$errno))
//			return false;
