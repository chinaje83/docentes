<?php 
abstract class cCiclosLectivosDB
{
	/** @var accesoBDLocal  */
	protected $conexion;
	/** @var mixed  */
	...
	protected $error;
	/**
	 * Constructor de la clase cCiclosLectivosDB.
	 *
	...
	/**
	 * Destructor de la clase cCiclosLectivosDB.
	 */
	function __destruct(){}
	...
	{
		$spnombre="sel_CiclosLectivos_xIdCicloLectivo";
		$sparam=array(
	...
	{
		$spnombre="sel_CiclosLectivos_busqueda_avanzada";
		$sparam=array(
	...
	{
		$spnombre="sel_CiclosLectivos_AuditoriaRapida";
		$sparam=array(
			'pIdCicloLectivo'=> $datos['IdCicloLectivo']
		);
		if(!$this->conexion->ejecutarStoredProcedure($spnombre,$sparam,$resultado,$numfilas,$errno) )
    ...
    {
        $spnombre="sel_CiclosLectivos_Combo";
        $sparam=array();
        if(!$this->conexion->ejecutarStoredProcedure($spnombre,$sparam,$resultado,$numfilas,$errno) )
        {
            FuncionesPHPLocal::MostrarMensaje($this->conexion,MSG_ERRGRAVE,"Error al buscar al buscar por codigo. ",array("archivo" => __FILE__,"funcion" => __FUNCTION__, "linea" => __LINE__),array("formato"=>$this->formato));
	...
	{
		$spnombre="upd_CiclosLectivos_xIdCicloLectivo";
		$sparam=array(
			'pNombre'=> $datos['Nombre'],
	...
	{
		$spnombre="del_CiclosLectivos_xIdCicloLectivo";
		$sparam=array(
			'pIdCicloLectivo'=> $datos['IdCicloLectivo']
		);
		if(!$this->conexion->ejecutarStoredProcedure($spnombre,$sparam,$resultado,$numfilas,$errno))
	...
	{
		$spnombre="upd_CiclosLectivos_Estado_xIdCicloLectivo";
		$sparam=array(
			'pEstado'=> $datos['Estado'],
			'pIdCicloLectivo'=> $datos['IdCicloLectivo']
		);
    ...
    protected function AsignarDefault(array $datos): bool
    {
        $spnombre="upd_CiclosLectivos_EsDefault";
        $sparam=array(
            'pIdCicloLectivo'=> $datos['IdCicloLectivo']
        );
        if(!$this->conexion->ejecutarStoredProcedure($spnombre,$sparam,$resultado,$numfilas,$errno))
    ...
    protected function DesasignarDefault(array $datos): bool
    {
        $spnombre="upd_CiclosLectivos_EsDefault_xIdCicloLectivo";
        $sparam=array(
            'pIdCicloLectivo'=> $datos['IdCicloLectivo']
        );
        if(!$this->conexion->ejecutarStoredProcedure($spnombre,$sparam,$resultado,$numfilas,$errno))
    ...
    protected function BuscarDefault(array $datos, &$resultado, &$numfilas)
    {
        $spnombre="sel_CiclosLectivos_EsDefault";
        $sparam=array(
            'pEsDefault' => 1,
            'pEstado' => ACTIVO
        );
